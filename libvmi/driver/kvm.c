...
{
    kvm_instance_t *kvm = kvm_get_instance(vmi);
    if ((kvm->shm_snapshot_fd = shm_open(kvm->shm_snapshot_path, O_RDONLY, (mode_t)0)) < 0) {
...
{
#ifdef DGVMA_TABLE
    destroy_v2m_table_list(&(kvm_get_instance(vmi)->shm_snapshot_v2m_tables));
#else
    destroy_v2m_rb_tree_list(&(kvm_get_instance(vmi)->shm_snapshot_v2m_rb_trees));
#endif
    kvm_teardown_shm_snapshot_mode(vmi);
...
size_t kvm_get_dgpma(
    ...
    if (VMI_SUCCESS == test_using_shm_snapshot(kvm_get_instance(vmi))) {
        *medial_addr_ptr = kvm_get_instance(vmi)->shm_snapshot_map + paddr;
        ...
        errprint("kvm_get_dgpma() failed because shm-snapshot is not using.\n");
        return 0;
    }
}
    ...
    size_t count)
{
    ...
    if (VMI_SUCCESS == test_using_shm_snapshot(kvm)) {
        ...
        status_t ret_search;
#ifdef DGVMA_TABLE
        ret_search = search_v2m_table(vmi, pid, &kvm->shm_snapshot_v2m_tables, vaddr,
            medial_addr_ptr, &v2m_size, kvm->shm_snapshot_fd);
#else
        ret_search = search_v2m_rb_tree(vmi, pid, &kvm->shm_snapshot_v2m_rb_trees, vaddr,
            medial_addr_ptr, &v2m_size, kvm->shm_snapshot_fd);
#endif

        if (VMI_FAILURE == ret_search) {
            return 0;
        }
    ...
    {
        errprint("kvm_get_dgvma() failed because shm-snapshot is not using.\n");
        return 0;
    }
}
