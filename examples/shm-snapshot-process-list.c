...
int main (int argc, char **argv)
{
    ...
    /* create a shared memory snapshot */
    if (vmi_snapshot_create(vmi) != VMI_SUCCESS) {
    ...
    /* destroy the shared memory snapshot, and return live mode */
    ...
    return 0;
#else
    printf("Error : this example should only run after ./configure --enable-snapshot.\n");
    return 1; // error
#endif
